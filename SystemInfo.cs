...
namespace Autoinventario
{
    ...
    public static class Systeminfo
    {
        ...
        private static bool IsRemoteControlPromptEnabled()
        {
            try
            {
                ...
                using (var key = Microsoft.Win32.Registry.LocalMachine.OpenSubKey(@"SYSTEM\CurrentControlSet\Control\Terminal Server"))
                {
                    if (key != null)
                    {
                        object? value = key.GetValue("RemoteControlPrompt");
        ...
        private static string GetPhysicalMemory()
        {
            ...
            int totalRamGB = (int)Math.Round(totalRam / (1024.0 * 1024 * 1024));
        ...
        private static string GetVirtualMemory()
        {
            ...
            if (obj != null)
            {
                ulong totalVirtualMemoryKB = Convert.ToUInt64(obj["TotalVirtualMemorySize"]);
                int totalVirtualMemoryGB = (int)Math.Round(totalVirtualMemoryKB / 1024.0 / 1024);
            ...
            try
            {
                using (var searcher = new ManagementObjectSearcher("SELECT SMBIOSMemoryType FROM Win32_PhysicalMemory"))
                {
                    foreach (var mo in searcher.Get().Cast<ManagementObject>())
                    {
                        ...
                        {
                            ushort smbiosType = Convert.ToUInt16(mo["SMBIOSMemoryType"]);
            ...
            using (var searcher = new ManagementObjectSearcher("SELECT * FROM Win32_DiskDrive"))
                foreach (var obj in searcher.Get().Cast<ManagementObject>())
                {
                    result.Add(new HardDisk
                    {
                        ...
                        capacity = obj["Size"] != null ? Convert.ToInt64(obj["Size"]) : (long?)null,
        ...
        static List<LogicalDrive> GetLogicalDrives()
        {
            ...
            foreach (var obj in searcher.Get().Cast<ManagementObject>())
            {
                result.Add(new LogicalDrive
                {
                    ...
                    capacity = obj["Size"] != null ? Convert.ToInt64(obj["Size"]) : (long?)null,
                    free_space = obj["FreeSpace"] != null ? Convert.ToInt64(obj["FreeSpace"]) : (long?)null
                });
            }
        ...
        static List<Mouse> GetMice()
        {
            ...
            foreach (var obj in searcher.Get().Cast<ManagementObject>())
            {
                ...
                string vid = Regex.Match(deviceId, @"VID[_&]?([0-9A-Fa-f]+)").Groups[1].Value;
                string pid = Regex.Match(deviceId, @"PID[_&]?([0-9A-Fa-f]+)").Groups[1].Value;

                string connectionType = GetConnectionType(deviceId);
        ...
        static Keyboard GetKeyboard()
        {
            ...
            if (obj != null)
            {
                string manufacturer = obj["Description"]?.ToString() ?? "N/A";
                string deviceId = obj["DeviceID"]?.ToString() ?? "N/A";
